          ...
          <ScrollReveal>
            ...
            {step === 1.5 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <Button variant="ghost" className="mb-4" onClick={() => setStep(1)}>← Back to role selection</Button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {[
                    ...
                    <Card 
                      key={item.id}
                      className={`cursor-pointer transition-all border-2 hover:border-primary group ${delegateType === item.id ? 'border-primary bg-primary/5' : 'border-border'}`}
                      onClick={() => handleDelegateTypeSelection(item.name)}
                    >
                      <CardContent className="p-8 text-center space-y-4">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto transition-colors ${delegateType === item.id ? 'bg-primary text-white' : 'bg-muted text-secondary group-hover:bg-primary/10 group-hover:text-primary'}`}>
                          {item.icon}
                        </div>
                        <h3 className="font-bold text-xl">{item.name}</h3>
                        <p className="text-sm text-muted-foreground">{item.desc}</p>
            ...
            {step === 2 && (
              <div className="bg-white border border-border p-10 rounded-[2rem] shadow-xl animate-in zoom-in-95 duration-500">
                <Button variant="ghost" className="mb-8" onClick={() => role === "delegate" ? setStep(1.5) : setStep(1)}>← Back</Button>
